            ...
            if (sideBar && items.length > 0) {
                for (i = 0; i < sideBar.childNodes.length; i++) {
                    ...
                    if (elem.className && Sys.UI.DomElement.containsCssClass(elem, 'About')) {
                        elem.getElementsByTagName('div')[1].innerHTML = Web.DataViewResources.Menu.About;
                        break;
                    }
                }
...
Web.HoverMonitor.prototype = {
    ...
    startClose: function () {
        this.stopClose();
        this._closeInterval = window.setInterval('var hm=Web.HoverMonitor._instance;hm.stopClose();hm.close()', 1000);
    },
    ...
    startCloseItem: function (family, path) {
        this.stopCloseItem();
        this._closeItemInterval = window.setInterval(String.format('Web.HoverMonitor._instance.closeItem("{0}","{1}")', family, path), 1000);
    },
    stopOpen: function () {
    ...
    startOpen: function (family, path) {
        ...
        if (!this._skipStartOpen) {
            this._openInterval = window.setInterval(String.format('Web.HoverMonitor._instance.open("{0}", "{1}")', family, path), Web.HoverMonitor._instance._tempOpenDelay != null ? Web.HoverMonitor._instance._tempOpenDelay : 500);
