...
namespace BUDI2_NS.Data
{
    ...
    public class Localizer
    {
        ...
        public static string JavaScriptStringEncode(string text)
        {
            return text.Replace("\r\n", "\\r\\n").Replace("\'", "\\\'").Replace("\"", "\\\"");
        }
        ...
        private string DoReplaceToken(Match m)
        {
            ...
            if (token == m.Groups[3].Value)
            {
                ...
                if (_scriptMode)
                	result = JavaScriptStringEncode(result);
                return result;
            }
            else
            	return m.Value;
        ...
        public static Stream CreateDictionaryStream(string culture, string baseName, string objectName, out string[] files)
        {
            ...
            Stream result = t.Assembly.GetManifestResourceStream(CultureManager.ResolveEmbeddedResourceName(String.Format("BUDI2_NS.{0}{1}", baseName, fileName), culture));
            if (result == null)
            	result = t.Assembly.GetManifestResourceStream(CultureManager.ResolveEmbeddedResourceName(String.Format("BUDI2_NS.{0}", fileName), culture));
            if (result == null)
            {
