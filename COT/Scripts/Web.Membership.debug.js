...
Web.Membership.prototype = {
    ...
    updated: function () {
        ...
        this._updateLastActivity();
        if (!String.isNullOrEmpty(this.get_cultures())) {
            this._cultureList = [];
            var cultures = String.format('Detect,Detect|{0}|False;{1}', Web.MembershipResources.Bar.AutoDetectLanguageOption, this.get_cultures());
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                Array.add(this._cultureList, cultureInfo);
                var item = new Web.Item(family, cultureInfo[1]);
                if (cultureInfo[2] == 'True')
                    item.set_selected(true);
                item.set_script(String.format('$find(\'{0}\').changeCulture(\'{1}\');', this.get_id(), cultureInfo[0]));
        ...
        var loggedIn = supportsIsLoggedIn ? Sys.Services.AuthenticationService.get_isLoggedIn() : true;
        ...
        if (!loggedIn && this.get_displayLogin()) {
            ...
            var b = $common.getBounds(this._loginDialog);
    ...
    _login_Completed: function (validCredentials) {
        if (!validCredentials) {
            ...
            window.location.replace(returnUrl ? unescape(returnUrl[1]) : (anchoredHref ? anchoredHref[1] : window.location.href));
        }
    },
