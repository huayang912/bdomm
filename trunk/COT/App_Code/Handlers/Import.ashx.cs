            ...
            if (context.Request.HttpMethod == "GET")
            	...
            	if ((context.Request.HttpMethod == "POST") && (context.Request.Files.Count > 0))
                {
                    ...
                    try
                    {
                        ...
                        data = String.Format("<form><input id=\"NumberOfRecords\" type=\"hidden\" value=\"{0}\"/><input id=\"Available" +
                                "ImportFields\" type=\"hidden\" value=\"{1}\"/><input id=\"FieldMap\" type=\"hidden\" valu" +
                                "e=\"{2}\"/><input id=\"FileName\" type=\"hidden\" value=\"{3}\"/></form>", numberOfRecords, HttpUtility.HtmlAttributeEncode(availableImportFields), HttpUtility.HtmlAttributeEncode(fieldMap), Path.GetFileName(tempFileName));
                    }
