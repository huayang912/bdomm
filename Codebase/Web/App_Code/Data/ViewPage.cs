...
namespace BUDI2_NS.Data
{
	public class ViewPage
    {
        ...
        protected object LoadStatefulObject(string cookie, string name)
        {
            if ((HttpContext.Current != null) && (HttpContext.Current.Session != null))
            	return HttpContext.Current.Session[name];
            return HttpRuntime.Cache[(cookie + name)];
        }
        
        protected void SaveStatefulObject(string cookie, string name, object instance)
        {
            if ((HttpContext.Current != null) && (HttpContext.Current.Session != null))
            	HttpContext.Current.Session[name] = instance;
            else
            {
                string cacheKey = (cookie + name);
                HttpRuntime.Cache.Remove(cacheKey);
                HttpRuntime.Cache.Add(cacheKey, instance, null, Cache.NoAbsoluteExpiration, TimeSpan.FromMinutes(20), CacheItemPriority.Normal, null);
            }
        }
        ...
        public DataTable ToDataTable(string tableName)
        {
            ...
            foreach (DataField field in Fields)
            	table.Columns.Add(field.Name, Controller.TypeMap[field.Type]);
