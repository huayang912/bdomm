...
function ShowModalPopup(popupId, title, message) 
{
    _PopupID = popupId,            
    _PopupTitle = title;
...
function ShowProgress() {    
    var blockingConatiner;
    var hasCreatedEarlier = false;
    if (null != document.getElementById('divProgressAnimation')) {
        blockingConatiner = document.getElementById('divProgressAnimation');
    }
    else {
        blockingConatiner = document.createElement('div');
        blockingConatiner.id = 'divProgressAnimation';
    }
    var documentHeight = $(document).height();
    var documentWidth = $(document).width();
    documentWidth = $.browser.msie ? (documentWidth - 22) : documentWidth;
    documentHeight = $.browser.msie ? (documentHeight - 6) : documentHeight;    
    blockingConatiner.style.position = 'absolute';
    blockingConatiner.style.width = documentWidth + 'px';
    blockingConatiner.style.height = documentHeight + 'px';
    blockingConatiner.style.left = 0 + 'px';
    blockingConatiner.style.top = 0 + 'px';
    blockingConatiner.style.zIndex = 30;
    blockingConatiner.className = 'progressanimation';
    document.body.appendChild(blockingConatiner);
    blockingConatiner.style.display = '';
    
    ///Show the Container
    $('#divProgressAnimationContainer').show();
}
function HideProgress() {
    $("#divProgressAnimation").css({ 'display': 'none' });
    $('#divProgressAnimationContainer').hide();
}
/** Progress Animation End */
/*Ajax Start*/
var _ErrorMessageClass = 'ErrorMessageBox';
var _SuccessMessageClass = 'MessageBox';
var DEFAULT_DLL_ELEMENT = '<option value=""></option>';        

function OnAjax_Error(error) {
    alert(error.get_message());
    HideProgress();
}
function OnAjax_TimeOut() {
    HideProgress();
    alert('Operation TimeOut');    
}
var CONFIRMATION_MSG_CUSTOM_ATTR = 'ConfirmationMessage';
function ShowSaveConfirmation(btnElement) {
    var validationGroup = $(btnElement).attr('ValidationGroupName');
    ...
    if (Page_IsValid) {
        return confirm($(btnElement).attr(CONFIRMATION_MSG_CUSTOM_ATTR));
    }
    return false;
}
/*Ajax End*/
...
$(document).ready(function() {
    var position = $('#childpagebody').position();
    if (position != null) {
        $('.SubMenuContainer').each(function() {
            $(this).css('left', position.left).css('top', position.top);
        });
        $('.SubMenuItemContainer').css('left', position.left).css('top', position.top);
    }
});

function ShowMenu(subMenuContainerID, liElement) {
    if (!$('#' + subMenuContainerID).is(':visible')) {
        ///Hide other Submenus First
        $('.SubMenuItemContainer').each(function() {
            $(this).fadeOut('slow', DetachClickEvent);
        });
        $('.SubMenuContainer').each(function() {
            $(this).fadeIn('slow');
        });
        $('#' + subMenuContainerID).fadeIn('slow', AttachClickEvent);
        $('.tabs li').removeAttr('style');
        //$(liElement).css('background-color', '#B32400').css('color', '#FFFFFF');
        $(liElement).css('background-image', 'url(/Images/SubMenu/MenuItemHover.gif)').css('color', '#FFFFFF');        
    }
}
function HideMenu() {
    $('.SubMenuItemContainer').each(function() {
...
function AttachClickEvent() {
    $(document).bind('click', function(e) {
        var targerElement = $(e.target);
        if (!(targerElement.is('.SubMenuItemContainer') || targerElement.parents().is('.SubMenuItemContainer'))) {            
            HideMenu();
        }        
    });
}
...
/* Submenu Section End */
var DATE_FORMAT = 'mm/dd/yy';//Default Value
function ValidateDate(sender, args) {
    args.IsValid = false;
    try {
        var dateToCheck = args.Value;
        dateToCheck = $.datepicker.parseDate(DATE_FORMAT, dateToCheck);
        args.IsValid = true;
    }
    catch (Error) {
        args.IsValid = false;
...
function SetHeight() {
    if (window.parent != null) {
        window.parent.SetFrameHeight(0);
...
function GetWords(text, wordCount) {    
    if (GetWordCount(text) > wordCount) {
...
function ConfirmDelete(url, message) {
    //ShowConfirmationPopup('Delete Data', message);
    //_DeleteURL = url;
    if (confirm(message) == true)
        window.location = url + '&Rnd=' + GetRandomNumber();
}
/**Ribbon**/
/*$(document).ready(function () {
    $('#divRibbonTabs .RibbonTab, #divRibbonTabs .RibbonTabSelected').each(function () {
        $(this).click(function () {
            var className = $(this).attr('class');
            if (className == 'RibbonTab') {
                $('#divRibbonTabs .RibbonTab, #divRibbonTabs .RibbonTabSelected').each(function () { $(this).removeClass('RibbonTabSelected').addClass('RibbonTab') });
                $(this).removeClass('RibbonTab').addClass('RibbonTabSelected');
                ///Hide All Other tabs
                $('#divRibbonItemContainer .RibbonItemCollection').hide();
                var id = '#' + $(this).attr('ItemsContainerID');
                $(id).show();
            }
        });
    });
});*/
function ShowRibbonTab(divTab, divId) {
    if (!$('#' + divId).is(':visible')) {
        $('.RibbonItemCollection').hide();
        $('#' + divId).show();
        $('#divRibbonTabs .RibbonTabSelected').removeClass('RibbonTabSelected').addClass('RibbonTab');
        $(divTab).removeClass('RibbonTab').addClass('RibbonTabSelected');
    }
}
/**Ribbon End**/